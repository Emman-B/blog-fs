...
const { getBlogPosts, createBlogPost, getBlogPost, updateExistingBlogPost, deleteBlogPost } = require('./blogposts');
const { createNewAccount, loginToAccount, authenticateTokenCookie, logoutAccount, getLoggedInUserInfo, checkTokenCookie, updateUserDetails } = require('./auth');
 ...
 */
// getting all blogposts (needs middleware to check optional authentication)
app.get('/v1/blogposts', checkTokenCookie, getBlogPosts);
// getting one blogpost by uuid (needs middleware to check optional authentication)
app.get('/v1/blogposts/:id', checkTokenCookie, getBlogPost);
// creating a new blog post (requires authentication middleware)
app.post('/v1/blogposts', authenticateTokenCookie, createBlogPost);
// updating an existing blog post (requires authentication middleware)
app.put('/v1/blogposts/:id', authenticateTokenCookie, updateExistingBlogPost);
// deleting a blog post (requires authentication middleware)
app.delete('/v1/blogposts/:id', authenticateTokenCookie, deleteBlogPost);
...
app.put('/v1/user', authenticateTokenCookie, updateUserDetails);
